...
As of 15 May 2024 this has 100% coverage for __init__.py. However, it could use some
...
@pytest.mark.xfail(raises=NotImplementedError)
def test_calcutor_protocol_run_method(test_calculator):
    test_calculator.run()


@pytest.mark.xfail(raises=NotImplementedError)
def test_calcutor_protocol_to_df_method(test_calculator):
...
def test_sliding_windows_with_different_alignment_modes(doc, generator_type, n=5):
    windows = sliding_windows(
        doc, n=n, window_units="characters", alignment_mode="expand"
    )
    assert type(windows) == generator_type
    windows_list = list(windows)
